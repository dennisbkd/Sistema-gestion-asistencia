...
class PeriodoAcademicoController extends Controller
{
    ...
    /**
     * Show detalle del periodo académico
     */
    public function detalle(Request $request, PeriodoAcademico $periodosAcademico)
    {
        $materiasQuery = Materia::whereHas('asignaciones', function($q) use ($periodosAcademico) {
            $q->where('idPeriodo', $periodosAcademico->idPeriodo);
        });

        $docentesQuery = Docente::whereHas('asignaciones', function($q) use ($periodosAcademico) {
            $q->where('idPeriodo', $periodosAcademico->idPeriodo);
        });

        $gruposQuery = Grupo::whereHas('asignaciones', function($q) use ($periodosAcademico) {
            $q->where('idPeriodo', $periodosAcademico->idPeriodo);
        });

        // Filtros interdependientes
        if ($request->filled('materia')) {
            $materiaId = $request->materia;
            $materiasQuery->where('idMateria', $materiaId);
            $docentesQuery->whereHas('asignaciones', fn($q) => $q->where('idMateria', $materiaId)->where('idPeriodo', $periodosAcademico->idPeriodo));
            $gruposQuery->whereHas('asignaciones', fn($q) => $q->where('idMateria', $materiaId)->where('idPeriodo', $periodosAcademico->idPeriodo));
        }

        if ($request->filled('docente')) {
            $docenteId = $request->docente;
            $docentesQuery->where('idDocente', $docenteId);
            $materiasQuery->whereHas('asignaciones', fn($q) => $q->where('idDocente', $docenteId)->where('idPeriodo', $periodosAcademico->idPeriodo));
            $gruposQuery->whereHas('asignaciones', fn($q) => $q->where('idDocente', $docenteId)->where('idPeriodo', $periodosAcademico->idPeriodo));
        }

        if ($request->filled('grupo')) {
            $grupoId = $request->grupo;
            $gruposQuery->where('idGrupo', $grupoId);
            $materiasQuery->whereHas('asignaciones', fn($q) => $q->where('idGrupo', $grupoId)->where('idPeriodo', $periodosAcademico->idPeriodo));
            $docentesQuery->whereHas('asignaciones', fn($q) => $q->where('idGrupo', $grupoId)->where('idPeriodo', $periodosAcademico->idPeriodo));
        }

        return Inertia::render('periodos-academicos/Detalle', [
            'periodo' => $periodosAcademico,
            'materias' => $materiasQuery->withCount(['asignaciones' => fn($q) => $q->where('idPeriodo', $periodosAcademico->idPeriodo)])->get(),
            'docentes' => $docentesQuery->with('usuario')->withCount(['asignaciones' => fn($q) => $q->where('idPeriodo', $periodosAcademico->idPeriodo)])->get(),
            'grupos' => $gruposQuery->withCount(['asignaciones' => fn($q) => $q->where('idPeriodo', $periodosAcademico->idPeriodo)])->get(),
            'filters' => $request->only(['materia', 'docente', 'grupo']),
        ]);
    }
}